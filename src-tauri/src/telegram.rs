...
mod tests {
    ...
    fn the_prepare_phase_cancel_reply_carries_a_measured_number() {
        ...
        assert!(
            CANCEL_PREPARE_WORST_SECS < 1.0,
            "الرقم المعلَن {CANCEL_PREPARE_WORST_SECS} ث خرج من نطاق «يُقتل فوراً» — سقفٌ مطّاط لا زمن مقيس"
        );
        eprintln!(
            "م٣/إلغاء التحضير: المقيس من مسار التنزيل {measured:.4} ث · المعلَن {CANCEL_PREPARE_WORST_SECS} ث · الجواب «{reply}»"
        );
    }
