...
mod tests {
    ...
    fn the_cancel_stop_is_typed_in_the_production_path() {
        ...
        // (٠) ثابت واحد للجملة: النصف الإنتاجي **لا يكتبها نصّاً** (بل يحملها
        // `pipeline::CANCELLED_BY_USER`). وهذا شرط **معنويّ لا شكليّ**: لا يفرض
        // صيغة بناء، فلا يرفض كوداً سليماً.
        assert!(
        ...
        //
        // **وشرطٌ شكليّ قديم أُزيل هنا — بقياس**: كان
        // `production.contains("return Err(SepError::Cancelled);")` على **نصف
        // الملف**، وهو **يرفض كوداً سليماً**: دالّةٌ مساعدة تبني النوع و`demix`
        // يستهلكه (شكلٌ مشروع) تُسقطه، **والحارس السلوكي ينجح** في 2.86 ث —
        // قِيس. وصيغةُ البناء ليست هي المدعيّة (المدّعى **الدلالة**)، والشرط
        // الجديد أدناه يحكم الدلالة ويرى العطل في كل صيغه.
        let code = collapsed(&code_only(production));

        // (١) **متن فرع التوقّف داخل `demix` بعينه** — لا في نصف الملف.
        let demix = block_after(&code, "fn demix(");
        let stop = block_after(&demix, "if !progress(");

        // (٢) **والدلالة لا الصيغة**: الفرع **لا يبلّغ عن فشل**. والبدائل الأربعة
        // المذكورة هي **بقيّة `SepError` كلها** — فلا يبقى بعدها إلا الإلغاء
        // (`SepError::Cancelled` أو دالّة مساعدة تُبنى منه، وكلاهما مشروع).
        // وهذا يحكم المعنى: `Err(SepError::Cancelled)` و`Err(cancelled())` يمرّان،
        // و`Err(SepError::Inference(…))` — العطل المقيس — يسقط **أياً كانت صياغة
        // جمله** (ولو جُزِّئت إلى نصّين أو كُتبت خامّة).
        for variant in ["Inference(", "Io(", "ModelMissing", "InvalidInput("] {
            assert!(
                !stop.contains(variant),
                "فرع التوقّف في `demix` يبلّغ عن فشل (`{variant}`) — والتوقّف إلغاءُ \
                 مستخدم لا عطل: {stop}"
            );
        }

        // (٣) **ولا فرعَ توقّفٍ ثانٍ مطابق**: الشكل المباشر يُذكر **مرّة واحدة على
        // الأكثر** (صفر = دالّة مساعدة بنَت النوع، وهي مشروعة)، وتكرارُ الفرع
        // بعينه في موضع آخر (هجوم A6b) يسقط.
        //
        // (وكان هنا `code.matches("SepError::Cancelled").count() == 1`: عدٌّ **كلّي**
        // أفرط ⇒ رفض موضعاً ثانياً مشروعاً ودالّةً مساعدة، والسلوكي ينجح — أي
        // **حارسٌ يرفض الصواب**، وهو ما لا يجوز.)
        assert!(
            code.matches(CANCEL_STOP_BRANCH).count() <= 1,
            "فرع التوقّف المباشر مكرَّر في النصف الإنتاجي — فرعٌ **مطابق** في دالّة \
             أخرى لا معنى له (والموضع الثاني المشروع شرطه مختلف فلا يُطابق النصّ)"
        );
    }

    /// الفرع الواحد الذي يحرسه هذا الاختبار — نصّه **مطويّ الفراغات**.
    const CANCEL_STOP_BRANCH: &str =
        "if !progress(done as f32 / total_steps as f32) { return Err(SepError::Cancelled); }";

    /// متن الكتلة `{ … }` التي **تلي** أوّل `needle` — **بموازنة الأقواس**.
    ///
    ...
    /// بالاكتفاء. فحُصر الحكم في **متن دالّة `demix`** ثم في **متن فرع التوقّف**
    /// داخلها.
    ///
    /// ويُشترط أن يكون `code` **مجرَّداً** (تعليقات وسلاسل مزالة): قوسٌ داخل نصّ
    /// أو تعليق يُفسد الموازنة.
    ///
    /// **حدّه (معلَن)**: أوّل `{` بعد `needle` هو بداية الكتلة، وموازنة العدّ لا
    /// تعرف `'{'` كمحرف — وهو غير مستعمل في هذا الملف.
    fn block_after(code: &str, needle: &str) -> String {
        let start = code
            .find(needle)
            .unwrap_or_else(|| panic!("«{needle}» غير موجود في نصّ الشيفرة المفحوص"));
        let open = start
            + code[start..]
                .find('{')
                .unwrap_or_else(|| panic!("لا كتلة بعد «{needle}»"));
    ...
    /// المُعاد ليس `Cancelled`. **ومُفسَد العمى**: حدُّ إلغاء مشروع **قبل**
    /// `demix` مع إعادة العطل ⇒ يسقط بعدّاد نداءات التقدّم (صفر) وبالأرضية
    /// الزمنية، بعد أن كان ينجح في 0.02 ث.
    #[test]
    fn a_cancelled_engine_call_returns_the_typed_cancel() {
        ...
        //
        // **ولا يُقبل نجاحٌ لم يُنفَّذ** (ثقب ثالث رصده الجاسوس): لو وقع «توقّف»
        // **قبل** بلوغ `demix` (حدُّ إلغاء مشروع في `separate` مثلاً) لنجح هذا
        // الحارس **وهو لم يرَ العطل** — قِيس في هجومه: نجاح في **0.02 ث** والعطل
        // قائم. فحارسان يمنعان العمى، **والحاكم منهما هو الثاني**:
        //  - **عدّاد نداءات التقدّم**: يقيس أن **نداء التقدّم سُئل فعلاً** (وإن
        //    كان حدٌّ مبكّر يسأله أيضاً — فهو إشارة عدم-فراغ ضعيفة لا برهان
        //    بلوغ `demix`؛ ووجودهما معاً مقصود).
        //  - **أرضية زمنية** (0.25 ث) — **وهي الفاصل الحقيقي**: بناء الجلسة +
        //    مقطع استدلال = **2.8 ث** مقيسة، والعودة المبكرة **23µs** مقيسة ⇒
        //    الفرق **أربع رتب**، والهامش ١١× على أسرع آلة معقولة (والأبطأ يزيده).
        ...
        assert!(
            ...
            "نداء التقدّم لم يُسأل ولا مرّة ⇒ هذا القياس لم يبلغ بروتوكول التقدّم \
             أصلاً (نجاحٌ في {elapsed:?}) — حارسٌ لم يُنفَّذ لا يُحتسب"
        );
        assert!(
            elapsed >= std::time::Duration::from_millis(250),
            "الحارس عاد في {elapsed:?} — أقلّ من زمن بناء الجلسة ومقطع استدلال \
             (المقيس 2.8 ث، والعودة المبكرة المقيسة 23µs) ⇒ الإلغاء سبق `demix` \
             فلم يُقَس ما يدّعيه الحارس"
        );
        let e = match res {
