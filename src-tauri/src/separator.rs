...
mod tests {
    ...
    fn a_cancelled_engine_call_returns_the_typed_cancel() {
        ...
        // قائم. فحارسان يمنعان العمى:
        //  - **عدّاد نداءات التقدّم**: `demix` وحده ينادي `progress` (بعد مقطع
        //    استدلال كامل، `session.run_model` ثم الفحص) ⇒ **صفر نداءات = لم
        //    يُبلَغ `demix`** ⇒ سقوط. وهذا **حتميّ لا زمنيّ**.
        //  - **أرضية زمنية** خشنة (0.25 ث) شبكةَ أمان: المقيس 2.8 ث ⇒ هامش ١١×.
        ...
        assert!(
            ...
            "نداء التقدّم لم يُسأل ولا مرّة ⇒ الإلغاء سبق بلوغ `demix`، فالحارس \
             **لم يقِس** ما يدّعيه (نجاحٌ في {elapsed:?}) — وهذا هو العمى المرفوض"
        );
        assert!(
            elapsed >= std::time::Duration::from_millis(250),
            "الحارس عاد في {elapsed:?} — أقلّ من زمن بناء الجلسة ومقطع استدلال \
             (المقيس 2.8 ث) ⇒ لم يُنفَّذ ما يدّعيه"
        );
        let e = match res {
