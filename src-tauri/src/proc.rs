...
/// يُضمّ كل خيوط القراءة المسجَّلة في سياق المهمّة الجارية (يُنادى عند انتهاء
/// المهمّة، فلا يبقى خيط قارئ على أنبوب طفل مقتول).
pub fn join_reader_threads(ctx: &JobCtx) {
    let threads = match ctx.phases.lock() {
        Ok(mut p) => std::mem::take(&mut p.threads),
        Err(p) => std::mem::take(&mut p.into_inner().threads),
    };
    for t in threads {
        let _ = t.join();
    }
