...
CASES.push({
  ...
  mutants: [
    ...
    /* الثقب المقيس: حارسٌ يُنزَع بـ`#[ignore]` واختبارٌ تافه يُضاف مكانه ⇒ العدد الكلي
       ...
       **ولا بدّ من `names` هنا** (وهو الخطأ الذي وقعتُ فيه في دمج `agent/dl2`): بلا جرد
       أسماء يسقط الحارس أولاً بـ«صفر مدخل: لم يُقرأ سطر اختبار واحد» ⇒ **لا يصل إلى
       مقارنة المُهمَل أصلاً**، فيسقط لسبب آخر و`mustMatch` لا يجده (قِيس:
       `مُفسَدات 80/81 · سقط لكن لم يسمِّ العيب`). فالبيئة هنا يجب أن تحمل الجرد
       **لتبلغ** المقارنةَ المقصودة — لا لتُسقط الحارس قبلها. */
    { label: 'حارس صار #[ignore] (واختبار تافه مكانه) ⇒ يسقط بالزيادة في المُهمَل',
      apply: (dir) => { relFakeCargo(dir, { warnings: 15, passed: 349, ignored: 5, names: relTestNames(349) }); },
      mustMatch: /اختبارات مُهمَلة: 5 > الأساس 4/ },
    /* **وضابطُ الحدّ نفسه**: البيئة **بلا** جرد أسماء ⇒ يسقط بـ«صفر مدخل» ولا يمرّ صامتاً.
       وهذه هي الحالة التي كانت ستُقرأ «نجاحاً» لو لم يُشترط مدخل غير فارغ. */
    { label: 'بلا جرد أسماء إطلاقاً ⇒ صفر مدخل يسمّي السبب (لا نجاح على لا شيء)',
      apply: (dir) => { relFakeCargo(dir, { warnings: 15, passed: 349 }); },
      mustMatch: /صفر مدخل: لم يُقرأ سطر اختبار واحد/ },
...
/* ═══ ٧) حارس «عقد الوكلاء محليّ» — نقلٌ من `agent/housekeeping` وإيصالٌ هنا ═══
 * **الفكرة**: دليل الوكلاء عقدٌ داخلي **لا يُنشر**، فلا يجوز أن يبقى في الشجرة
 * المنشورة مرجعٌ يقود قارئاً إلى ملفٍ ليس فيها. والحارس يقيس أربعة شروط,
 * **كلها مقيسة لا مُستنتجة**: (أ) الملف على القرص · (ب) غير متتبَّع
 * (`git ls-files` فارغ) · (ج) مُتجاهَل (‏`git check-ignore` يُطبع قاعدة)
 * · (د) **صفر مرجع حيّ**. ويُفرّق في (د) ثلاثة أصناف: **حيّ** (متابعة أو مؤشّر
 * لا يُحلّ) ⇒ يسقط · **تاريخيّ** في `docs/AUDIT.md` ⇒ مُعفى بالسطر والنصّ
 * (فتقادم الإعفاء نفسه يُسقط) · **تعليلٌ في شيفرة** ⇒ يُبلَّغ ولا يُسقط.
 *
 * **ومجرَّب في هذه الشجرة**: قِيس **٧ مراجع حيّة** أُصلحت كلها (لا استُثنيت):
 * `docs/RELEASE-0.2.9.md` ×٤ · `docs/STATUS.md` ×٢ (أحدهما رابط markdown حقيقي)
 * · `qa/TEST-MATRIX.md` ×١ — **وأحدها كان سطراً كتبتُه أنا** فكشفه الحارس عليّ.
 *
 * **وأُوصل هنا لأن الحارس خارج السجلّ يتبخّر** (اقتضاء `AGENT.md` §١٠): فيُقاس
 * بضابط ومُفسَدات، ومُفسَد (ب) هو ما يمنع عودة الملف إلى التتبّع صامتاً
 * (فقاعدة `.gitignore` وحدها **لا تُزيل التتبّع**).
 */
const AGENT_DOC_NAME = 'AGENT.md';
CASES.push({
  name: 'check-agent-doc-local.cjs',
  script: S('check-agent-doc-local.cjs'),
  // الحارس صار يقبل `--root` (كنمط بقية الحرّاس)، فيُقاس على البيئة المصنوعة
  // بلا نسخ: `copyInto` كانت تنسخه لكن حارس الحرّاس ينادي `c.script` من المستودع
  // ما لم تكن `needsScriptCopy`، وهي تنسخ `build-info`/`sbom` وحدهما.
  controlArgs: (dir) => ['--root', dir],
  build(dir) {
    copyInto(dir, 'scripts/check-agent-doc-local.cjs', S('check-agent-doc-local.cjs'));
    // مستودع git مصنوع: شرطَا (ب) و(ج) يقيسان `git ls-files` و`git check-ignore`،
    // **وشرط (د) يقيس أيضاً أن الإعفاء التاريخي لم يتقادم**: فـ`docs/AUDIT.md` لا بدّ
    // أن يكون **موجوداً ومتتبَّعاً ويحمل الإشارة**، وإلا سقط الحارس بـ
    // «stale historical exemption(s)» — وهو ما قِيس فعلاً في المحاولة الأولى.
    initRepo(dir);
    mk(dir, 'AGENT.md', '# the local contract\n\ncontent\n');
    mk(dir, '.gitignore', '/AGENT.md\n');
    mk(dir, 'README.md', '# published file\n\nno reference here\n');
    mk(dir, 'docs/AUDIT.md', '# audit\n\n- the contract AGENT.md was added on that date (historical record)\n');
    runGit(['add', 'README.md', '.gitignore', 'docs/AUDIT.md'], dir);
    runGit(['-c', 'user.email=q@q.q', '-c', 'user.name=q', 'commit', '-m', 'fixture'], dir);
  },
  /* **والعدّاد المقيس هو الإعفاء التاريخي المحفوظ** — لا عدد «المراجع الحيّة»، لأن
   * الأخير **صفر في البيئة السليمة وفي كل مُفسَد** (المُفسَد يُسقط الحارس، فلا معنى
   * لعتبته). والإعفاء التاريخي **واحد** في البيئة المصنوعة (`docs/AUDIT.md`)، وهو
   * **مدخل غير صفري يثبت أن الحارس قرأ الملفات فعلاً** ولم يمرّ على مدخل فارغ.
   * (والمحاولة الأولى استعملت مخرَج `--self-check` فلا يظهر في التشغيل العادي ⇒
   * قِيس `سaw=0` فسقط **الضابط** لا المُفسَد.) */
  saw: (dir, res) => { const m = res.out.match(/historical kept\s*:\s*(\d+)/); return m ? Number(m[1]) : 0; },
  sawExpected: 1,
  mutants: [
    { label: '(د) مرجع حيّ: ملف منشور يشير إلى العقد المحلي ⇒ يسقط ويسمّي الملف والسطر',
      apply: (dir) => mk(dir, 'README.md', '# published file\n\nsee ' + AGENT_DOC_NAME + ' §3 for the rules\n'),
      mustMatch: /live reference: README\.md/ },
    { label: '(د) رابط markdown إليه في ملف **متتبَّع** ⇒ مرجع حيّ ويسقط',
      // **ولا بدّ من `git add`**: الحارس يمسح **الملفات المتتبَّعة** وحدها
      // (`git ls-files -z`) — وهو الصواب، فالملف المنشور متتبَّع بالضرورة. وقِيس
      // أن مُفسَداً بملف **غير متتبَّع** يمرّ (‏`exit=0`) لأن الحارس **لا يراه أصلاً**
      // ⇒ فكان مُفسَداً باطلاً يُوهم بثقب.
      apply: (dir) => {
        mk(dir, 'docs/GUIDE.md', '# guide\n\n[the contract](../' + AGENT_DOC_NAME + ')\n');
        runGit(['add', 'docs/GUIDE.md'], dir);
      },
      mustMatch: /live reference: docs\/GUIDE\.md/ },
    { label: '(ب) الملف عاد إلى التتبّع (`git add`) ⇒ يسقط: قاعدة .gitignore لا تُزيل التتبّع',
      apply: (dir) => { runGit(['add', '-f', AGENT_DOC_NAME], dir); },
      mustMatch: /\(B\)|untracked/i },
  ],
  zero: { label: 'العقد غائب عن القرص ⇒ يسقط (عقدٌ مفقود لا يُقاس عليه)',
    apply: (dir) => fs.rmSync(path.join(dir, AGENT_DOC_NAME), { force: true }) },
});

/* ── **وحالةُ فاحص الإشعارات لم تُسجَّل — وهذا إعلانُ حدٍّ لا نسيان** ─────────────
 * الفاحص نفسه **يعمل ومُثبت بالمُفسَدات** (قِيس على الشجرة الحقيقية: الضابط
 * `exit=0` · وثلاثة مُفسَدات `exit=1` لكلٍّ — ومنها **مُفسَد C**: تبديل ترقيم
 * عنوان (`## ٢)` ⇒ `## ٩)`) **بلا تغيير أي عدّ**، وهو الثقب الذي لا يراه فاحص
 * الأعداد القائم). **لكن تسجيله في هذه الحارسة لم يُكمل**: سبع محاولات قِيست
 * وسقطت لأسباب متتالية — الجذر من `__dirname` · و`--root` الافتراضي الذي يمرّره
 * حارس الحرّاس فيُقرأ مساراً · وترقيم عربيّ-هندي لازم في الترجمة المصنوعة ·
 * وملفٌّ غير متتبَّع لا يراه الفاحص (وهو صواب).
 * ⇒ **وأُعلن ذلك بدل أن يُترك حالةً حمراء تُوهم**: الحالة أُزيلت من السجلّ،
 * و`pnpm notices:fidelity` **مُوصَّل في `package.json` ويعمل**، **والفاحصان
 * ومُفسَداتهما في `ARCHIVE/`** لمن يكمل. **وهذا حدّ مُعلَن لا ادّعاء تغطية.**
 * ─────────────────────────────────────────────────────────────────────────── */
