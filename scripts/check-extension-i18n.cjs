...
function auditAll(texts) {
  ...
  const ALLOWED_LATIN = [
    ...
    /* ط-١٢/البند ٤: رموز عطل المحرّك الفرعية (`sub.*`) تسمّي الأدوات والتقنية
     * بأسمائها — وهي **أسماء لا تُعرَّب** بنصّ القاعدة ① أعلاه، ولها **سابقة
     * مقيسة في المنتج نفسه**: جدول التطبيق العربي `src/i18n.ts` يحمل `yt-dlp`
     * داخل جمل عربية في **٨ مواضع** («تحديث yt-dlp الآن» · «النسخة المحلية:
     * yt-dlp {version}» …). فإبقاؤها لاتينية هنا اتّساقٌ مع المشحون لا توسيع. */
    { text: 'yt-dlp', why: 'أداة خارجية باسم لاتينيّ، وهو لاتينيٌّ في جدول التطبيق العربي أيضاً (٨ مواضع)' },
    { text: 'ffmpeg', why: 'أداة خارجية — اسمها العلم لاتينيّ ولا تُعرَّب' },
    { text: 'ffprobe', why: 'أداة خارجية من حزمة ffmpeg — اسمها لاتينيّ' },
    { text: 'ONNX', why: 'اسم صيغة/محرّك الاستدلال — مصطلح تقني لاتينيّ' },
  ];
