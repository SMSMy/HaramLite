 ...
 *
 * **بعد دمج م١ (0.2.9)**: أُسقطت قيود الملفات الخمسة (`bridge.rs` · `cli.rs` ·
 * `lib.rs` · `telegram.rs` · `watch_service.rs`) لأن مداخلها انتقلت إلى الغلاف
...
function main(argv) {
  ...
  /* سطر الحصيلة صار صريحاً بعد ملاحظة مدقّق مستقلّ: الصيغة السابقة
   * («منها 0 في مسار المنتج») كانت تُقرأ خطأً كأنها عن الشجرة كلها، وهي عن
   * **المواضع المباشرة** وحدها — ولا قيد `role:'منتج'` في القائمة اليوم فصفرها
   * **بنائيّ**. والصورة الكاملة تحتاج الشقّين: صفر مباشر في مسار المنتج،
