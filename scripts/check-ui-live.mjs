...
//
// ⚠ **وهذه الوصفة لا تعمل على ثنائي 0.2.9 — مقيسة 2026-09-25**: المنفذ 9222 لم يستمع رغم
//   ضبط المتغيّر بطريقتين مختلفتين (`$env:` قبل `Start-Process`، و`Start-Process -Environment`)
//   وبعد إغلاق كل النسخ. **والسبب الجذري**: سطر أوامر عملية WebView2 التابعة للتطبيق لا يحمل
//   `--remote-debugging-port` إطلاقاً ⇒ **العلم لا يصل إلى WebView2**. والتطبيق أقلع فعلاً
//   (نافذة + عمليات WebView2 + إقلاع في السجلّ) لكن بلا منفذ ⇒ **لا هدف CDP ⇒ لا قياس**.
//   ⇒ فهذه البوّابة **غير قابلة للقياس بهذه الطريقة على 0.2.9+**، و**لا تُعلَن خضراء بلا هدف**.
//   والمطلوب: قناة تنقيح يعرفها Tauri v2، أو سياسة `AdditionalBrowserArguments` في السجلّ.
