...
/** **المفاتيح الميتة المُعلَنة بالاسم** — **فارغة الآن، وهذا هو المقصود.**
 *
 * في جولة الحُزَم الأولى قِيس **26 مفتاحاً ميتاً** (277 مفتاحاً · 251 مستهلكاً)
 * وأُعلنت هنا بالاسم بدل حذفها، لأن الحذف كان خارج التكليف. ثم أُذن بالحذف
 * فحُذفت كلها من `src/i18n.ts` (الجدولين)، **فصارت القائمة فارغة**.
 *
 * **ولماذا نُبقيها فارغة ولا نحذفها**: هي **منفذ الإعلان**. وإبقاؤها فارغة يعني أن
 * أي مفتاح ميت جديد **يُسقط الحارس فوراً** بلا قائمة استثناء تُخفي شيئاً — وهو
 * الوضع الأقوى. ولو حُذفت القائمة نفسها لزال الفحص الذي يقابل المجموعة المُعلَنة
 * بالواقع في الاتجاهين.
 *
 * **وحدّ مُعلَن**: «ميت» هنا = لا مستهلك **بإشارة** (نصّ حرفيّ · ربط سمة · الموضع
 * المحسوب). ومفتاح يقرؤه فرعٌ لا يُنفَّذ يبقى «مستهلكاً» — ولهذا كان الحكم في
 * المفاتيح الثلاثة (`quality_same` · `preview_hint_song` · `preview_hint_clip`)
 * مقيساً بالاسم لا بالوصول، وهي **باقية** لأن شيفرتها الحاملة (الميتة) يملكها
 * عامل آخر. */
const KNOWN_DEAD: ReadonlyArray<{ key: string; why: string }> = [
  // فارغة عمداً — انظر الشرح أعلاه: أي مدخل هنا هو استثناء مُعلَن، ولا استثناء اليوم.
];
...
describe('ط-٤ · حارس المستهلك يرى الخطأ (مُفسَدات)', () => {
  ...
  it('مُفسَد (هـ): إعلانٌ لمفتاح صار مستهلكاً ⇒ الإعلان يشيخ فيُسقط', () => {
    // **والقائمة الحقيقية فارغة اليوم** (حُذفت المفاتيح الـ26)، فلو بقي هذا الفحص
    // على `actions_title` لصار **فحصاً ميتاً يقيس مفتاحاً لم يعد موجوداً**. فالخاصية
    // تُقاس بإعلان مصنوع: نُعلن مفتاحاً **حيّاً** ونتحقّق أن الفحص يكشفه.
    const victim = 'set_notify';
    expect(RESULT.consumed, 'الضحيّة حيّة').toContain(victim);
    const fakeDeclaration = new Set([victim]);
    const revived = [...fakeDeclaration].filter((k) => RESULT.consumed.includes(k));
    expect(revived, 'الفحص يكشف إعلاناً شاخ').toEqual([victim]);
  });

  it('وضابط: اليتيم صفر والإعلان فارغ — فلا استثناء قائماً', () => {
    // هذا هو الوضع الذي حُذفت المفاتيح لأجله: **لا مفتاح بلا مستهلك ولا استثناء**.
    // ولو عاد مفتاح ميت سقط الفحص الذي قبله، ولو شاخ إعلان سقط هذا.
    expect(KNOWN_DEAD, 'مدخلات الإعلان').toHaveLength(0);
    expect(RESULT.orphans, 'مفاتيح بلا مستهلك').toHaveLength(0);
  });
