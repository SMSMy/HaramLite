 ...
 * المنفصلة (`ffmpeg`/`yt-dlp`) تُقتل فوراً، ونداء المحرّك داخل العملية
 * **لا يُقطع** بل يُهجَر عند أول حدّ. فالواجهة تقول حدّاً، ولا تقول «فوراً».
 *
 * الرقم **مقيس لا مخترع**: أطول نداء محرّك مسجَّل في سجلّ المالك
 * (`%LOCALAPPDATA%\com.harammute.haramlite\logs\haramlite.log.2026-09-21`) هو
 * **565.451 ث** (`inference_ms=565451`، أكبر ١١ عيّنة في ذلك اليوم: التالي
 * 250377 ثم 179368، والوسيط نحو 32.7 ث) ⇒ **566** ث بترفيع إلى الثانية.
 * ولذلك نصّ الواجهة يقول «أطول ما قيس».
 *
 * **وسابقة مُصلَحة (م٢/إصلاح)**: كان هنا ١٤١ وفي تلغرام ١٥٠ — **رقمان مختلفان
 * لنفس الحالة**، وكلاهما من عيّنة `inference_ms=140208` قديمة ⇒ وعدٌ أقصر من
 * الواقع **٣.٨×** (‏566/150). والرقم الآن **واحد** في Rust
 * (‏`src-tauri/src/pipeline.rs: ENGINE_CALL_CEILING_SECS`) وهنا، وحارس تكافؤ
 * يمنع تباعدهما: `src/__tests__/stopCeiling.parity.test.ts`.
 *
 * **حدّ أمانة**: هذا أطول ما قيس، **لا حدّ رياضي** — نداء أطول (ملفّ أطول، أو
 * بناء تصحيح: قيس 33.5 ث لملف 60 ث مقابل 3.7 ث في بناء الإصدار) يتجاوزه.
 * فالرقم سقف على المقيس، والصدق في وصفه لا في الرقم وحده. */
export const STOP_CEILING_SECS = 566;
