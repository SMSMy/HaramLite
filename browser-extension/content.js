...
(() => {
  ...
  const I18N = {
    ...
    en: {
      ...
      'watch.mapMissing': '✗ The processed audio has no timing map — request processing again',
   ...
   *     `{type:'link', url, watch:true, mode:MODE}` ⇒ التطبيق يأخذ
   *     **المسار المؤقّت** (`bridge.rs:1067-1071`: `page_audio_dir()` — صوتي فقط،
   *     ولا شيء في مجلد المستخدم) **بالوضع المعروض نفسه**. فلا تتراكم ملفات في
   *     `~/Videos/HaramLite` من زرّ المشاهدة، **ولا يخالف المُرسَل المعروض**.
   ...
   * يقبل `watch:true` مع `mode` — والمالك نصّ على ذلك: «أو مع `mode` كما هو؛ الشقّ
   * الآخر سيقبل الاثنين»). **و`mode` يُرسَل في العقدين**: هو ما يجعل «المعروض =
   * المُرسَل» مصدراً واحداً بعد العطل المقيس (تعليق الطلب أدناه). */
  async function startFull(kind) {
    ...
    try {
      ...
      // **وكلاهما يحمل `mode` من الحالة لحظة الإرسال**. والسبب **عطل ميداني مقيس**
      // (بلاغ المالك 2026-09-23): زرّ المعالجة كان يرسل العلم **بلا `mode`**، فيقع
      // التطبيق على إعداده الداخلي — `bridge.rs:1108-1112` ⇒ `s.watch_mode`،
      // **وافتراضيّه `"song"`** (`settings.rs:80`) — بينما المعروض في قائمتنا `clip`
      // (افتراضيّ هذا الملف) ⇒ «يعالج كـ`song` والمعروض `clip`».
      // والقياس الحيّ **قبل** الإصلاح: وضعان معروضان مختلفان (`clip` ثم `song`، بلا لمس
      // القائمة) أرسلا **الحمولة نفسها** `{watch:true}` بلا `mode` ⇒ المعروض لم يكن
      // يُقرأ أصلاً. وبعد الإصلاح: المعروض والمُرسَل **مصدر واحد** (`MODE`)،
      // ويحرسه صفّان في القياس الحيّ (‏`clip` بلا نقرة ⇒ `mode:"clip"`، و`song` بلا
      // نقرة ⇒ `mode:"song"`) فلا يُثبَّت أحدهما عشوائياً.
      const req = (kind === 'save')
        ? { type: 'link', url: location.href, mode: MODE }
        : { type: 'link', url: location.href, watch: true, mode: MODE };
      r = await native(req);
    } catch (e) {
