...
> `store-assets/` (خارج git). **المنشور في المتجر: 1.1.1 علناً** — بوابة الرفع انفتحت (وافقت قوقل على النسخة الأولى). **1.1.2 جاهزة محلياً** (إصلاح زر المشاهدة + أيقونة جديدة + نافذة جديدة + اختيار الوضع في النافذة) وتُرفع كتحديث للعنصر القائم، لأن المتجر يرفض إعادة رفع النسخة نفسها.
...
1. [ ] رفع `HaramLite-Bridge-1.1.2-chrome.zip` (يُبنى بـ`pnpm pack:ext`؛ المانيفست في الجذر، الإصدار 1.1.2، **بلا حقل `key`**) — **كتحديث للعنصر القائم** (لا عنصر جديد ولا `Unlisted` أولاً — النسخة الأولى اعتُمدت ونُشرت علناً).
...
6. [ ] بعد النشر: تثبيت نسخة المتجر على جهاز فيه التطبيق **0.2.1 أو أحدث**، ثم إرسال رابط — نجاح التكامل يعني أن معرّف المتجر (`kaijaffkolenjhfcbaepmjndheahhikg`) مقبول.
