...
> `store-assets/` (خارج git). **المعلَّق في المتجر: 1.1.0** — ولا يُعدَّل حتى تُوافق قوقل على النسخة الأولى. **1.1.2 جاهزة محلياً** (إصلاح زر المشاهدة + أيقونة جديدة + نافذة جديدة + اختيار الوضع في النافذة) وتُرفع بعد الموافقة، لأن المتجر يرفض إعادة رفع النسخة نفسها.
...
1. [ ] رفع `HaramLite-Bridge-1.1.2-chrome.zip` (يُبنى بـ`pnpm pack:ext`؛ المانيفست في الجذر، الإصدار 1.1.2، **بلا حقل `key`**) — كعنصر جديد بظهور **Unlisted** أولاً.
