...
| ١٤ | **حارس مدخل الفصل الواحد** | `pnpm separation:entry` | كل **ذكر حيّ** للمعرّف `process_file` في مصادر Rust يُقابَل بقائمة مسموحة صريحة (ملف + **عدد متوقَّع**): **٧ مواضع** اليوم (٥ في مسار المنتج · ٢ في الاختبارات) وغلاف العامل (`slots.rs`/`jobs.rs`) يُسمح بواحد عند وجوده. وُلد لأن حماية 0.2.9 (محدِّد فتحات الفصل) تقوم على أن **كل** المداخل تمرّ عبر غلاف واحد — فحارسٌ يمنع **الثامن** أرخص من اكتشافه بعد البناء عليه. المطابقة **واسعة عمداً** وعلى النصّ **بعد نزع التعليقات**: تمسك `p::process_file(` بعد `use … as p` والنداء العاري بعد `use …::process_file` ومؤشّر الدالة — لا صيغةً بعينها. ويفشل (exit 2) على **صفر موضع** أو مجلد مصادر مفقود: «صفر مدخل ليس نجاحاً». المقيس (2026-09-20): **٧ مواضع مسموحة · ٠ غير مسموح** | `Separation entry guard (one entry, one wrapper)` |
| ١٥ | **حارس الحرّاس** | `pnpm guards:selfcheck` | تبني لكل حارس من **أحد عشر** بيئةً مصنوعة تحت `%TEMP%` فيها **مُفسَد** (يجب أن يُسقطه) و**ضابط** (يجب أن يمرّ عليه وقد رأى مدخلاً غير صفري) و**صفر مدخل** (يجب أن يفشل بصوت عالٍ). المقيس (2026-09-20): **ضوابط 11/11 · مُفسَدات 38/38 · صفر مدخل 11/11** — والزمن **8.5 ث و15.5 ث** في تشغيلين (يتغيّر بحمل الجهاز، فليُقرأ مدىً لا رقماً). وهذا هو **الوجه الثالث** لقاعدة المستودع: «حارس لا يرى ليس حارساً» **تُطبَّق على الحرّاس أنفسهم** | `Guard self-check gate (mutant + control per guard)` |
