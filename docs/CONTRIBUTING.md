...
- الاختبار اليدوي المقابل لها: `docs/TEST-MATRIX.md` §٧.
...
ويُربط بالبلاغ في تعليق. وإن كان الاختبار يدوياً فيُدرج صفّه في `docs/TEST-MATRIX.md`.
...
- **والربط بـCI شرط** (`AGENT.md` §١٠): اختبار لا يُشغَّل تلقائياً ليس حارساً. وإن تعذّر
  ...
  `docs/TEST-MATRIX.md` **بديل مُعلَن لا مكافئ**.
...
## البوابات (تسع — تُشغَّل بهذا الترتيب)
...
|---|---|---|---|---|
| ١ | اختبارات Rust | `cargo test --quiet` | **١٧٠ ناجحاً + ٣ مُهمَل** من ١٧٣ (مقيس 2026-09-15 بـ`cargo test --lib`): الفصل بالنموذج الحقيقي · الوسائط · الإعدادات · الجسر · ٦ اختبارات لحالات CUDA · واختبارا E2E (أحدهما `#[ignore]` يحتاج ffmpeg) | `ci.yml:79-81` |
| ٢ | clippy | `cargo clippy --all-targets` | **خطأ = فشل** — والسير بلا `-D warnings` عمداً كي لا تتحوّل التحذيرات المتبقّية (كلها استثناءات موثَّقة في `docs/AUDIT.md`) إلى عطل البوابة | `ci.yml:83-85` |
| ٣ | TypeScript | `pnpm exec tsc --noEmit` | سلامة أنواع الواجهة (`src/` — ١٨ ملفاً بعد تقسيم البند ١٢) | `ci.yml:87-88` |
| ٤ | **بناء الواجهة** | `pnpm build:web` | أن الحزمة **تُبنى فعلاً** (tailwind ثم Rollup): يُسقطها استيراد مفقود أو وحدة معطوبة — وهي ما يُشحن. أُضيفت في هذه الموجة لأن CI لم يكن يبني الواجهة إطلاقاً (`tsc` يتحقّق من الأنواع ولا ينتج حزمة). الزمن المقيس **2.5s** | `ci.yml:94-95` |
| ٥ | تكافؤ الإعدادات | `pnpm settings:parity` | تطابق مفاتيح الإعدادات بين الواجهة والـRust (‏22=22) — ويتّبع `collectSettings()` حيث كانت في `src/**/*.ts` ويفشل عند صفر تعريف أو تعريفين | `ci.yml:97-98` |
| ٦ | حراسة الإضافة | `pnpm ext:guard` | ‏473 فحصاً على `browser-extension/content.js` (موضع الصوت الواحد · منع السحب للخلف · السلوك الإجباريّ · قرار الفجوة · وضع التسريع) | `ci.yml:105-106` |
| ٧ | **بوابة المُفسَدات (سلبية)** | `pnpm ext:mutants` | ٢٧ مُفسَداً سلوكياً يُطبَّق على الملف المشحون **في الذاكرة**، وكل مُفسَد **يجب** أن يُسقط الحارس | `ci.yml:111-112` |
| ٨ | اتساق الإصدار | `pnpm versions:check` | إصدار التطبيق في أربعة ملفات (`package.json` · `tauri.conf.json` · `Cargo.toml` · `Cargo.lock`) — يفشل عند أول انحراف، ويطبع إصدار الإضافة **معلَماً بدورة مستقلة** (`1.1.5`) فلا يوحَّد معه | `ci.yml:116-117` |
| ٩ | حرّاس الموقع | `pnpm site:check` | خمسة حرّاس على `docs/`: CSS · الصفحات · الوسوم · العربي · الروابط | `ci.yml:119-120` |

> **البوابة السابعة سالبة بالمعنى الدقيق: النجاح فيها ليس «الحارس أخضر» بل «الحارس يرى».**
